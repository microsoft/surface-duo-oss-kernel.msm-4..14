...
static void himax_mcu_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size; page_prog_start += FLASH_RW_MAX_LEN) {
		...
		if (page_prog_start < 0x100) {
			...
			if (page_prog_start >= 0x100 && page_prog_start < 0x10000) {
				tmp_data[3] = 0x00;
				tmp_data[2] = 0x00;
				...
				if (page_prog_start >= 0x10000 && page_prog_start < 0x1000000) {
					tmp_data[3] = 0x00;
...
{
	/* 0x10007f00 -> 0x00009AA9 //Diable Flash Reload*/
...
void himax_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
	...
	*/
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			g_core_fp.fp_flash_write_burst_lenth (tmp_addr, &(tmp_data[start_index+i * max_bus_size]),  max_bus_size);
			total_size_temp = total_size_temp - max_bus_size;
		} else {
			I ("last total_size_temp=%d\n", total_size_temp);
			g_core_fp.fp_flash_write_burst_lenth (tmp_addr, &(tmp_data[start_index+i * max_bus_size]),  total_size_temp % max_bus_size);
		}
