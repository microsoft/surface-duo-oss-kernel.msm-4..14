...
static const struct regmap_range_cfg wcd9335_ifc_dev_ranges[] = {
	{
...
static struct regmap_config wcd9335_ifc_dev_regmap_config = {
	.reg_bits = 16,
	.val_bits = 8,
	.can_multi_write = true,
	.max_register = WCD9335_REG(0, 0x7FF),
	.ranges = wcd9335_ifc_dev_ranges,
	.num_ranges = ARRAY_SIZE(wcd9335_ifc_dev_ranges),
};
			       ...
			       enum slim_device_status status)
{
	...
	wcd->ifc_dev_regmap = regmap_init_slimbus(wcd->slim_ifc_dev,
						  &wcd9335_ifc_dev_regmap_config);
	if (IS_ERR(wcd->ifc_dev_regmap)) {
		dev_err(wcd->dev, "Failed to allocate register map\n");
