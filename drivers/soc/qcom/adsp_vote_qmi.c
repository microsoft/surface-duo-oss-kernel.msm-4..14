...
static int pgs_vote_send_sync_msg(struct qmi_handle *dev,int vote)
{
	...
	resp = kzalloc(sizeof(struct prod_set_lpm_vote_resp_msg_v01), GFP_KERNEL);
	if (!resp) {
		kfree(req);
		return -ENOMEM;
	}

	req->sleep_vote = (u8) vote;
	...
	if (!vote)
		pr_err("ADSP manual unvoting is successful\n");
	else
		pr_err("ADSP manual voting is successful\n");
...
static int send_adsp_manual_unvote(struct device *dev)
{
	...
	pr_err("ADSP unvoting initiated\n");
				 ...
				 struct qmi_service *service)
{
	...
	sq.sq_port = service->port;
        ...
        pdev = platform_device_alloc("qmi_adsp_vote_client", PLATFORM_DEVID_AUTO);
	if (!pdev)
		return -ENOMEM;
...
static int adsp_manual_vote_driver_probe(struct platform_device *pdev){
	...
	if (ret < 0)
        ...
        pr_err("ADSP vote device is registered successfully\n");
        return ret;
err_handle:
        kfree(dev);
        return ret;
...
static int adsp_manual_vote_driver_remove(struct platform_device *pdev){
        struct qmi_handle *dev = (struct qmi_handle *) platform_get_drvdata(pdev);
        qmi_handle_release(dev);
        return 0;
}

static const struct of_device_id adsp_manual_vote_dev_dt_match[] = {
       { .compatible = "qcom,adsp_manual_vote", },
        {}
};

MODULE_DEVICE_TABLE(of, adsp_manual_vote_dev_dt_match);

static struct platform_driver adsp_manual_vote_dev = {
        ...
        .driver = {
                ...
                .of_match_table = adsp_manual_vote_dev_dt_match,
        },
};
...
{
        int ret = 0;
        ret = platform_driver_register(&adsp_manual_vote_dev);
        if (ret){
...
static void __exit adsp_manual_vote_qmi_deinit(void)
{
        ...
        platform_driver_unregister(&adsp_manual_vote_dev);
}
