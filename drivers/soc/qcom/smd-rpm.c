...
static int qcom_ipc_rpm_callback(void *dev,
				 const void *data,
				 size_t count)
{
	const struct qcom_rpm_header *hdr = data;
	struct qcom_ipc_device *qidev = dev;
	size_t hdr_length = le32_to_cpu(hdr->length);
	const struct qcom_rpm_message *msg;
	struct qcom_smd_rpm *rpm = dev_get_drvdata(&qidev->dev);
...
static int qcom_ipc_rpm_probe(struct qcom_ipc_device *idev)
{
...
static void qcom_ipc_rpm_remove(struct qcom_ipc_device *idev)
{
	of_platform_depopulate(&idev->dev);
}

static const struct of_device_id qcom_ipc_rpm_of_match[] = {
...
MODULE_DEVICE_TABLE(of, qcom_ipc_rpm_of_match);

static struct qcom_ipc_driver qcom_ipc_rpm_driver = {
	.probe = qcom_ipc_rpm_probe,
	.remove = qcom_ipc_rpm_remove,
	.callback = qcom_ipc_rpm_callback,
	.driver  = {
		.name  = "qcom_ipc_rpm",
		.owner = THIS_MODULE,
		.of_match_table = qcom_ipc_rpm_of_match,
	},
};

static int __init qcom_ipc_rpm_init(void)
{
...
{
	qcom_ipc_driver_unregister(&qcom_ipc_rpm_driver);
}
