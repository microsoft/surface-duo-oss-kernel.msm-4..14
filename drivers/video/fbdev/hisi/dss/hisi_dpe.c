...
static int dpe_lcd_fps_updt_handle(struct platform_device *pdev)
{
	...
	if (is_mipi_video_panel(hisifd)) {
		outp32(ldi_base + LDI_DPI0_HRZ_CTRL0, pinfo->ldi_updt.h_front_porch
			| ((pinfo->ldi_updt.h_back_porch + DSS_WIDTH(pinfo->ldi_updt.h_pulse_width)) << 16));
...
static int dpe_regulator_clk_irq_setup(struct platform_device *pdev)
{
	...
	if (hisifd->dpe_irq) {
		...
		if (ret != 0) {
			...
			printk(KERN_ERR "super-shi wwwwwwwwwwwwwwwww request_irq dpe_irq failed\n");
			return ret;
		} else {
			printk(KERN_ERR "super-shi wwwwwwwwwwwwwwwww request_irq dpe_irq success\n");
			disable_irq(hisifd->dpe_irq);
		}
	}
	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		if (hisifd->mmbuf_asc0_irq) {
			...
			printk(KERN_ERR "super-shi mmbuf_asc0_irq is null\n");
		}
	}
