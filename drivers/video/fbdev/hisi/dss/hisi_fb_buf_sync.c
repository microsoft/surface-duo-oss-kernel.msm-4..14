		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			if (layer->img.shared_fd >= 0) {
				...
				if (IS_ERR(ionhnd)) {
					ionhnd = NULL;
					HISI_FB_ERR
					    ("fb%d, layer_idx%d, failed to ion_import_dma_buf, ionclient %p, share_fd %d!\n",
					     hisifd->index, i,
					     hisifd->ion_client,
					     layer->img.shared_fd);
				} else {
			...
			if (add_tail) {
				...
				if (g_debug_layerbuf_sync) {
					HISI_FB_INFO
					    ("fb%d, frame_no=%d, layer_idx(%d), shared_fd=%d, ion_handle=%p, "
			    ...
			    struct list_head *pfree_list)
{
	...
	list_for_each_entry_safe(node, _node_, pfree_list, list_node) {
		...
		if (g_debug_layerbuf_sync) {
			HISI_FB_INFO
			    ("fb%d, frame_no=%d, share_fd=%d, ion_handle=%p, has_map_iommu=%d, "
...
void hisifb_buf_sync_unregister(struct platform_device *pdev)
{
	...
	if (hisifd->buf_sync_ctrl.timeline) {
		sync_timeline_destroy((struct sync_timeline *)hisifd->
