	...
	char __iomem *mipi_dsi_base)
{
	...
	// check DPHY clock lane phy_lock
	try_times = 0;
	tmp = inp32(mipi_dsi_base + MIPIDSI_PHY_STATUS_OFFSET);
	while ((tmp & BIT(0)) != 0) {
		udelay(10);
		if (++try_times > 100) {
			HISI_FB_ERR("fb%d, check DPHY clock lane phy_lock failed! MIPIDSI_PHY_STATUS=0x%x.\n",
				hisifd->index, tmp);
			break;
		}
	...
	// set PERIPH PERDIS3 bit[31:28] to 1
	...
	char __iomem *mipi_dsi_base)
{
	...
	HISI_FB_DEBUG("fb%d, +!\n", hisifd->index);
#if defined(CONFIG_HISI_FB_3660)
	// set PERIPH PEREN3 bit[31:28] to 1
	set_reg(hisifd->peri_crg_base + PEREN3, 0x1, 4, 28);

	// enable DPHY PLL, force_pll = 0
