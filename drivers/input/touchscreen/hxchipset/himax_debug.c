...
static ssize_t himax_diag_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		...
		if (storage_type > 0 && storage_type < 8 && rawdata_type > 0 && rawdata_type < 8) {
			I("%s,dsram\n", __func__);
			diag_max_cnt = 0;
		...
		if (buf[0] == 'v') { /* firmware version */
			...
			if (buf[0] == 'd') { /* ic information */
				...
				if (buf[0] == 't') {
					...
					if (buf[0] == 'i' && buf[1] == '2' && buf[2] == 'c') { /* i2c commutation */
						debug_level_cmd = 'i';
						return len;
					}

					else
						if (buf[0] == 'i' && buf[1] == 'n' && buf[2] == 't') { /* INT trigger */
							debug_level_cmd = 'n';
							return len;
						}
...
#if defined(CONFIG_TOUCHSCREEN_HIMAX_ITO_TEST)
	himax_proc_ito_test_file = proc_create(HIMAX_PROC_ITO_TEST_FILE, (S_IRWXO | S_IRWXU | S_IRWXG),
