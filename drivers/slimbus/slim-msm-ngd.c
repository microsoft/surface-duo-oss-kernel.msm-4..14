...
static int ngd_xfer_msg(struct slim_controller *ctrl, struct slim_msg_txn *txn)
{
	...
	//FIXME Is this possible in upstream ???
	//
	if (!report_sat) {
		...
		if ((pm_runtime_enabled(dev->dev) && ret < 0) ||
				dev->state >= MSM_CTRL_ASLEEP) {
			pr_err( "slim ctrl vote failed ret:%d, state:%d",
					ret, dev->state);
	...
	if (txn->mt == SLIM_MSG_MT_CORE &&
		...
		if (ret) {
			pr_err( "TID for connect/disconnect fail:%d\n",
					ret);
			goto ngd_xfer_err;
		}
		txn->len = i;
	...
	if (!pbuf) {
		pr_err( "Message buffer unavailable\n");
		ret = -ENOMEM;
		goto ngd_xfer_err;
	}
	dev->err = 0;
		...
		if (txn->mc != SLIM_USR_MC_DISCONNECT_PORT)
			...
			writel_relaxed(0, PGD_PORT(PGD_PORT_CFGn,
					(dev->pipes[wbuf[1]].port_b),
						dev->ver));
		if (dev->err) {
			pr_err( "pipe-port connect err:%d\n", dev->err);
			goto ngd_xfer_err;
		}
	...
	if (ret) {
		...
		SLIM_WARN(dev, "conf:0x%x,stat:0x%x,rxmsgq:0x%x\n",
				conf, stat, rx_msgq);
		pr_err( "int_stat:0x%x,int_en:0x%x,int_cll:0x%x\n",
				...
				struct slim_msg_txn *txn)
{
	...
	if (ret) {
		if (ret != -EREMOTEIO || txn->mc != SLIM_USR_MC_CHAN_CTRL)
			pr_err( "master msg:0x%x,tid:%d ret:%d\n",
				txn->mc, txn->tid, ret);
		spin_lock_irqsave(&ctrl->txn_lock, flags);
		ctrl->txnt[txn->tid] = NULL;
		spin_unlock_irqrestore(&ctrl->txn_lock, flags);
...
static int ngd_slim_power_up(struct msm_slim_ctrl *dev, bool mdm_restart)
{
	...
	if (!mdm_restart &&
		...
		ret = qcom_slim_qmi_power_request(&dev->qmi, true);
		if (ret) {
...
//		ret = msm_slim_qmi_init(dev, false);
		...
		if (!ret) {
			...
			pr_err("qmi init fail, ret:%d, state:%d\n",
					ret, dev->state);

	} else {
		//FIXME ....msm_slim_qmi_exit(dev);
	}
...
static int ngd_slim_power_down(struct msm_slim_ctrl *dev)
{
	...
	spin_unlock_irqrestore(&ctrl->txn_lock, flags);
	pr_emerg("DEBUG::::::::::::::::::::::::::::::; %s: %d\n", __func__, __LINE__);
	return qcom_slim_qmi_power_request(&dev->qmi, false);
}
#endif
...
static int ngd_notify_slaves(void *data)
{
	...
	int ret, i = 0;
#if 0
	//No need it now as we would get this from 
...
/* Register QMI Interest.. here */
	...
	///FIXME ...ngd_slim_enable(dev, true);
	qcom_slim_qmi_init(dev, true);
