...
void msm_slim_tx_msg_return(struct msm_slim_ctrl *dev, int err)
{
	...
	if (dev->use_tx_msgqs != MSM_MSGQ_ENABLED) {
		...
		if (dev->wr_comp[0]) {qrtr-cfg 1 && qrtr-ns
...
//			dev_err(dev->dev, "MC: %x, asdaMT: %x\n", mc, mt);
		}

	//	dev_err(dev->dev, "%s: msg len %d \n", __func__, msg_len);
...
static int qcom_slim_qmi_send_power_request(struct msm_slim_qmi *slimbus,
				struct slimbus_power_req_msg_v01 *req)
{
	...
	ret = qmi_txn_init(&slimbus->qmi, &txn, slimbus_power_resp_msg_v01_ei, &resp);
	if (ret < 0) {
		pr_err( "failed to allocate QMI txn\n");
		return ret;
	}

	ret = qmi_send_request(&slimbus->qmi, &slimbus->saddr, &txn,
			    	SLIMBUS_QMI_POWER_REQ_V01,
			 	SLIMBUS_QMI_SELECT_INSTANCE_REQ_MAX_MSG_LEN,
				slimbus_power_req_msg_v01_ei, req);
	if (ret < 0) {
		pr_err( "failed to send powerup request\n");
		qmi_txn_cancel(&txn);
		return ret;
	}
	...
	if (ret < 0)
		pr_err( "failed receiving QMI response\n");
	else if (resp.resp.result)
		pr_err( "Power up request failed\n");
	else
		pr_err( "Power uP REQUEST completed\n");
	
	return 0;
}

int qcom_slim_qmi_power_request(struct msm_slim_qmi *slimbus, bool active)
{
	struct slimbus_power_req_msg_v01 req;
	...
	return qcom_slim_qmi_send_power_request(slimbus, &req);
}

int qcom_slim_qmi_check_framer_request(struct msm_slim_qmi *slimbus)
{
	struct slimbus_chkfrm_resp_msg resp = { { 0, 0 } };
	struct qmi_txn txn;
	int ret;

	ret = qmi_txn_init(&slimbus->qmi, &txn, slimbus_chkfrm_resp_msg_v01_ei, &resp);
	if (ret < 0) {
		pr_err( "failed to allocate QMI txn\n");
		return ret;
	}
	...
	ret = qmi_send_request(&slimbus->qmi, &slimbus->saddr, &txn,
			    	SLIMBUS_QMI_CHECK_FRAMER_STATUS_REQ, 0 , NULL, NULL);
//			 	SLIMBUS_QMI_SELECT_INSTANCE_REQ_MAX_MSG_LEN,
//				slimbus_chkfrm_resp_msg_v01_ei, req);
	if (ret < 0) {
		pr_err( "failed to send chk framer status request\n");
		qmi_txn_cancel(&txn);
		return ret;
	}
	...
	if (ret < 0)
		...
		pr_err( "check framer status request failed\n");
	else
		pr_err( "Check framer status completed\n");
	
	return 0;
}

extern int ngd_slim_enable(struct msm_slim_ctrl *dev, bool enable, int ret);
static int qcom_slim_qmi_send_select_inst_req(struct msm_slim_qmi *slimbus,
				struct slimbus_select_inst_req_msg_v01 *req)
{
	struct slimbus_select_inst_resp_msg_v01 resp = { { 0, 0 } };
	struct qmi_txn txn;
	int ret;
	struct msm_slim_ctrl *ctrl = container_of(slimbus, struct msm_slim_ctrl, qmi);

	ret = qmi_txn_init(&slimbus->qmi, &txn, slimbus_select_inst_resp_msg_v01_ei, &resp);
	if (ret < 0) {
		pr_err( "failed to allocate QMI txn\n");
		return ret;
	}
	...
	if (ret < 0) {
		pr_err( "failed to send shutdown request\n");
		qmi_txn_cancel(&txn);
		return ret;
	}
	...
	if (ret < 0)
		...
		pr_err( "Select instance failed\n");
	else
		pr_err( "Select Instance completed\n");
	

	
	ngd_slim_enable(ctrl, true, resp.resp.result);

	return 0;
}

static void qcom_slim_select_work(struct work_struct *work)
{
	int ret;
	struct msm_slim_qmi *slimbus = container_of(work, struct msm_slim_qmi, select_work);
	struct slimbus_select_inst_req_msg_v01 req;
...
//	else
	...
	qcom_slim_qmi_send_select_inst_req(slimbus, &req);

//	msm_slim_qmi_power_request(slimbus, true);
//
//	msm_slim_qmi_check_framer_request(slimbus);
}

static void qcom_slim_del_server(struct qmi_handle *qmi, struct qmi_service *svc)
{

}

static void qcom_slim_net_reset(struct qmi_handle *qmi)
{
	struct msm_slim_qmi *slimbus = container_of(qmi, struct msm_slim_qmi, qmi);

	schedule_work(&slimbus->net_reset_work);
}

static int qcom_slim_new_server(struct qmi_handle *qmi, struct qmi_service *svc)
{
	struct msm_slim_qmi *slimbus = container_of(qmi, struct msm_slim_qmi, qmi);
	

	slimbus->saddr.sq_family = AF_QIPCRTR;
	slimbus->saddr.sq_node = svc->node;
	slimbus->saddr.sq_port = svc->port;
	svc->priv = slimbus;

	schedule_work(&slimbus->select_work);

	return 0;
}

const struct qmi_ops q_ops = {
	.new_server = qcom_slim_new_server,
	.del_server = qcom_slim_del_server,
	.net_reset = qcom_slim_net_reset,
};

static void qcom_slim_net_reset_work(struct work_struct *work)
{
	int ret;
	struct msm_slim_qmi *slimbus = container_of(work, struct msm_slim_qmi, net_reset_work);

	qmi_handle_release(&slimbus->qmi);

	ret = qmi_handle_init(&slimbus->qmi, SLIMBUS_QMI_POWER_REQ_MAX_MSG_LEN, &q_ops, NULL);
	if (ret < 0) {
		kfree(slimbus);
		return;
	}
	...
	slimbus->qmi.ops.new_server = qcom_slim_new_server;
	slimbus->qmi.ops.del_server = qcom_slim_del_server;
	slimbus->qmi.ops.net_reset = qcom_slim_net_reset;

	qmi_add_lookup(&slimbus->qmi, SLIMBUS_QMI_SVC_ID, 0, 0);

}


int qcom_slim_qmi_init(struct msm_slim_ctrl *ctrl, bool apps_is_master)
{
	int ret;
	struct qmi_handle *qmi = &ctrl->qmi.qmi;

	INIT_WORK(&ctrl->qmi.net_reset_work, qcom_slim_net_reset_work);
	INIT_WORK(&ctrl->qmi.select_work, qcom_slim_select_work);

	ret = qmi_handle_init(qmi, SLIMBUS_QMI_POWER_REQ_MAX_MSG_LEN, &q_ops, NULL);
	if (ret < 0) {
		return ret;
	}

	
	qmi->ops.new_server = qcom_slim_new_server;
	qmi->ops.del_server = qcom_slim_del_server;
