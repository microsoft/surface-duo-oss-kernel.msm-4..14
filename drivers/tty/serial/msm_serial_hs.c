...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	if (unlikely(line < 0 || line >= 2)) {
		printk(KERN_ERR "Invalid plaform device ID = %d\n", pdev->id);
		return -ENXIO;
	}

	dev_info(&pdev->dev, "msm_serial_hs: detected port #%d\n", line);

	msm_uport = &q_uart_port[line];
	uport = &msm_uport->uport;
	...
	if (!request_mem_region(uport->mapbase, size, "msm_serial_hs"))
		return -EBUSY;
...
/*
	...
	if (unlikely(!resource))
		return -ENXIO;
*/
	msm_uport->dma_tx_channel = 7;
	msm_uport->dma_rx_channel = 6;
...
/*
	...
	if (unlikely(!resource))
		return -ENXIO;
*/
	msm_uport->dma_tx_crci = 6;
	msm_uport->dma_rx_crci = 11;
