...
#define HISILOG_TAG switch_usb_class
HISILOG_REGIST();
...
void usb_custom_acc5_event(int pedestal_attach)
{
    ...
    hisilog_info("%s: sent uevent %s\n", __func__, envp[0]);

    if (ret < 0){
        hisilog_err("%s: uevent sending failed!!! ret=%d\n", __func__, ret);
    }else{
        hisilog_info("%s: uevent sending ok USB_CUSTOM_ACC5=custom_acc5_mode\n", __func__);
    }
}
...
{
    hisilog_info("%s: swstate_value=%d\n", __func__, swstate_value);
    switch (swstate_value){
        ...
        case USB_OFF:
            return FSA9685_OPEN;
        default:
            hisilog_err("%s: swstate_value[%d] is error!!!\n", __func__, swstate_value);
            return FSA9685_OPEN;
    }
}
...
void switch_usb2_access_through_ap(void)
{
    ...
    if (ret < 0)
        hisilog_err("%s: fsa9685_manual_sw error!!! ret = %d\n", __func__, ret);
}
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if (switch_usb_set_state(sdev, state) < 0) {
        hisilog_info("%s: switch_usb_set_state err\n", __func__);
        return -EINVAL;
    }
...
static void set_reg_to_target_state(int state)
{
    ...
    case USB_OFF:
        ...
        break;
    default:
        hisilog_err("%s: state[%d] is error!!!\n", __func__, state);
        break;
    }
    hisilog_info("%s: ------end.\n", __func__);
}
...
static int switch_usb_set_state(struct switch_usb_dev *sdev, int state)
{
    ...
    {
        hisilog_err("%s: swstate[%d] is invalid!!!\n", __func__, state);
        ret = -EINVAL;
        return ret;
    }
    ...
    if (sdev->state == state)
    {
        hisilog_info("%s: swstate is not changed. sdev->state[%d] and state[%d]\n", __func__, sdev->state, state);
        ret = -EINVAL;
        return ret;
    }
...
static int create_switch_usb_class(void)
{
    ...
    if (NULL==switch_usb_class) {
        hisilog_info("%s: switch_usb_class=NULL and class_create will start.\n", __func__);
        switch_usb_class = class_create(THIS_MODULE, "usbswitch");
        if (IS_ERR(switch_usb_class)){
            hisilog_err("%s: class_create error!!! switch_usb_class=%p\n", __func__, switch_usb_class);
...
static int switch_usb_dev_register(struct switch_usb_dev *sdev)
{
    ...
    if (IS_ERR_OR_NULL(switch_usb_class)) {
        hisilog_info("%s: switch_usb_class is ERR or NULL, and create_switch_usb_class will start.", __func__);
        ret = create_switch_usb_class();
        if (ret < 0){
            hisilog_err("%s: create_switch_usb_class error!!! sdev->name=%s\n", __func__, sdev->name);
            goto err_create_file;
        }
    }
    hisilog_info("%s: device_create will start.", __func__);
    sdev->dev = device_create(switch_usb_class, NULL,
        MKDEV(0, 0), NULL, "usbsw");
    if (IS_ERR(sdev->dev))
    {
        ret = PTR_ERR(sdev->dev);
        hisilog_err("%s: device_create error!!! sdev->name=%s\n", __func__, sdev->name);
        goto err_create_file;
    }
...
static const struct of_device_id platform_usbswitch_ids[] = {
    { .compatible = "hisilicon,usbswitch" },
    {},
};
...
static int usb_switch_probe(struct platform_device *pdev)
{
    ...
    if (ret < 0){
        hisilog_err("%s: switch_usb_dev_register error!!!\n", __func__);
        goto err_switch_usb_dev_register;
    }

    ret = device_create_file(sdev.dev, &dev_attr_swstate);
    if (ret < 0){
        hisilog_err("%s: device_create_file error!!!\n", __func__);
        goto err_switch_usb_dev_register;
    }
