...
static int MSEHwGetFaultStatus(struct MSEHw_Config *MSEHw_Config)
{
    ...
    pr_debug("Pack 1 Active Fault: %d   Pack 2 Active Fault: %d", 
...
int MSEHwConfigureCharging(struct MSEHw_Config *MSEHw_Config, uint32_t *pMSEChgMaxCurrent, uint32_t *pMSEChgFloatVoltage)
{
    ...
    {
        if((MSEPackRTStatus[FGAUGE_001].CFETOn) && (0 <= IBATT1))
        {
        ...
        else if((MSEPackRTStatus[FGAUGE_002].CFETOn) && (0 <= IBATT2))
        {
...
int MSEHwBalanceChargeCurrent(struct MSEHw_Config *MSEHw_Config, uint32_t *pMSEChgMaxCurrent)
{
    ...
    {
        pr_debug("Skip charge current balancing when a weak psu is active or when in autorecharge or when active thermal throttling :%d", *pMSEChgMaxCurrent);
        goto Exit;
    }
