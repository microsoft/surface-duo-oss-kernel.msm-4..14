...
static void RestoreFromFileBackup(struct BATTERY_PROTECTION *BatteryProtection)
{
    ...
    for(CurrentPack = 0; CurrentPack < NumPacks; CurrentPack++)
    {
        ...
        else if (BatteryTelemetry.FileRestore[CurrentPack])
        {
            ...
            BatteryTelemetryHoursToSecondsVector(BatteryProtectionBSCConfig[CurrentPack].hvtMediumHighTempRsocBuckets_sec, BatteryTelemetry.BatteryTelemetryBinary[CurrentPack].BscTelemetry.BSCRsocBucketsMediumLowTemp, HVT_RSOC_BUCKETS);
