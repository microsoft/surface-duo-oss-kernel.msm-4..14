...
static const struct hisi_fixed_factor_clock hi3660_crg_fixed_factor_clks[] = {
	{ HI3660_CLK_FACTOR_UART3, "clk_gate_uart3", "clk_ppll0", 1, 8, 0, },
	{ HI3660_CLK_FACTOR_MMC, "clk_factor_mmc", "clkin_sys", 1, 6, 0, },
	{ HI3660_CLK_GATE_I2C0, "clk_gate_i2c0", "clk_fll_src", 1, 1, 0, },
	{ HI3660_CLK_GATE_I2C1, "clk_gate_i2c1", "clk_fll_src", 1, 1, 0, },
	{ HI3660_CLK_GATE_I2C2, "clk_gate_i2c2", "clk_fll_src", 1, 1, 0, },
...
static void hi3660_clk_pmuctrl_init(struct device_node *np)
{
	...
	clk_data = hisi_clk_init(np, HI3660_PMUCTRL_NR_CLKS);
	if (!clk_data)
		return;
...
static void hi3660_clk_pctrl_init(struct device_node *np)
{
	...
	clk_data = hisi_clk_init(np, HI3660_PCTRL_NR_CLKS);
	if (!clk_data)
		return;
...
static void hi3660_clk_sctrl_init(struct device_node *np)
{
	...
	clk_data = hisi_clk_init(np, HI3660_SCTRL_NR_CLKS);
	if (!clk_data)
		return;
	hisi_clk_register_gate_sep(hi3660_sctrl_gate_clks,
...
static void hi3660_clk_crgctrl_init(struct device_node *np)
{
	...
	clk_data = hisi_clk_init(np, HI3660_CRGCTRL_NR_CLKS);
	if (!clk_data)
		return;
	...
	hisi_clk_register_fixed_rate(hi3660_fixed_rate_clks,
				     ...
				     clk_data);
	hisi_clk_register_gate_sep(hi3660_crgctrl_gate_clks,
