...
[EMAC_HW_vMAX][RGMII_IO_MACRO_DLL_POR_NUM_OF_REGS][2] = {
	[EMAC_HW_None] = {
		...
		RGMII_IO_MACRO_CONFIG_RGOFFADDR_OFFSET, 0x0 },
		[SDCC_HC_REG_DLL_CONFIG_POR_ARR_INDEX] = {
		SDCC_HC_REG_DLL_CONFIG_RGOFFADDR_OFFSET, 0x0 },
		[SDCC_HC_REG_DDR_CONFIG_POR_ARR_INDEX] = {
		SDCC_HC_REG_DDR_CONFIG_RGOFFADDR_OFFSET, 0x0 },
		[SDCC_HC_REG_DLL_CONFIG_2_POR_ARR_INDEX] = {
		SDCC_HC_REG_DLL_CONFIG_2_RGOFFADDR_OFFSET, 0x0 },
		[SDCC_USR_CTL_POR_ARR_INDEX] = {
		SDCC_USR_CTL_RGOFFADDR_OFFSET, 0x0 },
		[RGMII_IO_MACRO_CONFIG_2_POR_ARR_INDEX] = {
		RGMII_IO_MACRO_CONFIG_2_RGOFFADDR_OFFSET, 0x0 },
	},
	[EMAC_HW_v2_0_0] = {
		[RGMII_IO_MACRO_CONFIG_POR_ARR_INDEX] = {
	...
	[EMAC_HW_v2_2_0] = {
		...
		[SDCC_HC_REG_DLL_CONFIG_POR_ARR_INDEX] = {
		SDCC_HC_REG_DLL_CONFIG_RGOFFADDR_OFFSET, 0x6004642C },
		[SDCC_HC_REG_DDR_CONFIG_POR_ARR_INDEX] = {
		SDCC_HC_REG_DDR_CONFIG_RGOFFADDR_OFFSET, 0x00000000 },
		[SDCC_HC_REG_DLL_CONFIG_2_POR_ARR_INDEX] = {
		SDCC_HC_REG_DLL_CONFIG_2_RGOFFADDR_OFFSET, 0x00200000 },
...
int DWC_ETH_QOS_rgmii_io_macro_sdcdc_init(struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	while (1) {
		...
		if (VARDLL_LOCK == 1) {
			EMACDBG(
			"DLL lock status bit set. DLL init successful\n");
			break;
		}
...
int DWC_ETH_QOS_rgmii_io_macro_init(struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	case RGMII_MODE:
		...
		case SPEED_1000:
			...
			if (pdata->io_macro_tx_mode_non_id) {
				...
				RGMII_CONFIG_2_DATA_DIVIDE_CLK_SEL_UDFWR
					(rgmii_data_divide_clk);
				...
				/* RGMII_TX_POS and RGMII_TX_NEG input
				 * pins are swapped
				...
				/* If data arrives at positive
				 * edge or if data is
				 * delayed by 1.5ns/ 2ns then
				 * write 1 to RX_PROG_SWAP
...
#else
				RGMII_DDR_MODE_UDFWR(0x0);
				if(pdata->io_loopback_mode){
					RGMII_PROG_SWAP_UDFWR(0x1);
				}
				RGMII_BYPASS_TX_ID_EN_UDFWR(0x0);
				RGMII_CONFIG_2_TX_CLK_PHASE_SHIFT_EN_UDFWR(0x0);
				RGMII_BYPASS_FUNC_CLK_EN_UDFWR(0x1);
#endif
				RGMII_LOOPBACK_EN_UDFWR(loopback_mode_en);
			}
			break;
	...
	case RMII_MODE:
		...
		RGMII_CONFIG_2_RERVED_CONFIG_16_EN_UDFWR(0x0);
		RGMII_LOOPBACK_EN_UDFWR(loopback_mode_en);
#ifdef DWC_ETH_QOS_EMULATION_PLATFORM
		RGMII_DDR_MODE_UDFWR(0x1);
		RGMII_BYPASS_FUNC_CLK_EN_UDFWR(0x1);
		RGMII_GPIO_CFG_RX_INT_UDFWR(0x0);
		RGMII_POS_NEG_DATA_SEL_UDFWR(0x1);
		RGMII_CONFIG_2_TX_CLK_PHASE_SHIFT_EN_UDFWR(0x1);
		RGMII_CONFIG_2_TX_TO_RX_LOOPBACK_EN_UDFWR(0x0);
		RGMII_CONFIG_2_DATA_DIVIDE_CLK_SEL_UDFWR(0x1);
		RGMII_CONFIG_2_CLK_DIVIDE_SEL_UDFWR(0x0);
#endif
...
int DWC_ETH_QOS_rgmii_io_macro_dll_reset(struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	for (index_of_reg = 0 ; index_of_reg <
		RGMII_IO_MACRO_DLL_POR_NUM_OF_REGS; index_of_reg++) {
