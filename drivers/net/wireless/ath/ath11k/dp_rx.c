			       ...
			       struct hal_sw_mon_ring_entries *sw_mon_entries)
{
	...
	do {
		rx_msdu_link_desc = (u8 *)pmon->link_desc_banks[sw_cookie].vaddr +
		...
		for (i = 0; i < num_msdus; i++) {
			...
			if (!msdu) {
				ath11k_dbg(ar->ab, ATH11K_DBG_DATA,
					   "data full mon msdu pop has invalid buf_id %d\n",
					    buf_id);
				spin_unlock_bh(&rx_ring->idr_lock);
				break;
			}
			...
			if (drop_mpdu) {
				ath11k_dbg(ar->ab, ATH11K_DBG_DATA,
					   "data full mon i %d drop msdu %p ppdu_id %x\n",
...
next_msdu:
		...
		ret = ath11k_dp_rx_monitor_link_desc_return(ar,
							    p_last_buf_addr_info,
							    dp->mac_id);
		if (ret)
			ath11k_dbg(ar->ab, ATH11K_DBG_DATA,
