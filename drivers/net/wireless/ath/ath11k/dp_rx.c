					     ...
					     int *budget, struct sk_buff_head *skb_list)
{
	...
	dp = &ar->dp;
	rx_ring = &dp->rx_mon_status_refill_ring[ath11k_hw_mac_id_to_srng_id(&ab->hw_params, mac_id)];
...
int ath11k_dp_rx_process_mon_status(struct ath11k_base *ab, int mac_id,
				    struct napi_struct *napi, int budget)
{
	struct ath11k *ar = ab->pdevs[ath11k_hw_mac_id_to_pdev_id(&ab->hw_params, mac_id)].ar;
...
int ath11k_dp_process_rxdma_err(struct ath11k_base *ab, int mac_id, int budget)
{
	...
	ar = ab->pdevs[ath11k_hw_mac_id_to_pdev_id(&ab->hw_params, mac_id)].ar;
	err_ring = &ar->dp.rxdma_err_dst_ring[ath11k_hw_mac_id_to_srng_id(&ab->hw_params, mac_id)];
	rx_ring = &ar->dp.rx_refill_buf_ring;
