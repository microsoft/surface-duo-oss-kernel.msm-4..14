...
tcl_ring_sel:
	...
	if (ar->ab->hw_params.misc_caps & MISC_CAPS_TCL_0_ONLY)
		ti.ring_id = 0;
	else
	...
	if (!hal_tcl_desc) {
		 ...
		 */
		if (ring_map != (BIT(DP_TCL_NUM_RING_MAX) - 1) &&
		    !(ar->ab->hw_params.misc_caps & MISC_CAPS_TCL_0_ONLY)) {
			tcl_ring_retry = true;
			ring_selector++;
		}
				     ...
				     struct htt_rx_ring_tlv_filter *tlv_filter)
{
	...
	/* KVALO_FIXME: hack to workaround broken pktlog handling on
	 * QCA61390
	 */
...
int ath11k_dp_tx_htt_monitor_mode_ring_config(struct ath11k *ar, bool reset)
{
	...
	ret = ath11k_dp_tx_htt_rx_filter_setup(ab, ring_id, dp->mac_id,
