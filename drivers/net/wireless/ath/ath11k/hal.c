		...
		if (srng_config->lmac_ring) {
			...
			if (!ab->hw_params.shadow_support)
				...
				ath11k_dbg(ab, ATH11k_DBG_HAL,
					   "type:%d, num:%d, target_reg:0x%x, shadow:0x%x\n",
					   type, ring_num,
					   reg_base,
					   (u32)((void *)srng->u.src_ring.hp_addr -
					   (void *)ab->mem));
		}
	} else {
		...
		if (srng_config->lmac_ring) {
			...
			if (!ab->hw_params.shadow_support)
				...
				ath11k_dbg(ab, ATH11k_DBG_HAL,
					   ...
					   HAL_REO1_RING_HP(ab)),
					   (u32)((void *)srng->u.dst_ring.tp_addr -
					   (void *)ab->mem));
		}
	}
					  ...
					  int ring_num)
{
	...
	if (srng_config->ring_dir == HAL_SRNG_DIR_DST)
		srng->u.dst_ring.tp_addr = HAL_SHADOW_REG(shadow_cfg_idx) +
		ab->mem;
	else
		srng->u.src_ring.hp_addr = HAL_SHADOW_REG(shadow_cfg_idx) +
		ab->mem;
}
...
/* it must be called with srng->lock being hold */
