...
void dw_pcie_dma_set_wr_remote_done_int(struct dw_pcie *pci, u64 val)
{
	...
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_CH_CONTROL1) | 0x10,
...
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_CH_CONTROL1) | 0x8,
		PCIE_DMA_CH_CONTROL1);
}
...
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_MASK) & (~val),
...
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_MASK) & ((~val)<<16),
		PCIE_DMA_WRITE_INT_MASK);
	return 0;
}
...
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_READ_INT_MASK) & (~val),
...
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_READ_INT_MASK) & ((~val)<<16),
		PCIE_DMA_READ_INT_MASK);
	return 0;
}
...
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_MASK) | val, PCIE_DMA_WRITE_INT_MASK);
	return 0;
}

int dw_pcie_dma_set_wr_abort_int_mask(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_MASK) | val<<16, PCIE_DMA_WRITE_INT_MASK);
	return 0;
}

int dw_pcie_dma_set_rd_done_int_mask(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_MASK) | val, PCIE_DMA_READ_INT_MASK);
	return 0;
}

int dw_pcie_dma_set_rd_abort_int_mask(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_MASK) | val<<16, PCIE_DMA_READ_INT_MASK);
	return 0;
}

int dw_pcie_dma_clear_wr_done_int(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_CLEAR) | val, PCIE_DMA_WRITE_INT_CLEAR);
	return 0;
}

int dw_pcie_dma_clear_wr_abort_int(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_WRITE_INT_CLEAR) | val<<16, PCIE_DMA_WRITE_INT_CLEAR);
	return 0;
}

int dw_pcie_dma_clear_rd_done_int(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_READ_INT_CLEAR) | val, PCIE_DMA_READ_INT_CLEAR);
	return 0;
}

int dw_pcie_dma_clear_rd_abort_int(struct dw_pcie *pci, u64 val)
{
	dw_pcie_writel_rc(pci, dw_pcie_readl_dbi(pci, PCIE_DMA_READ_INT_CLEAR) | val<<16, PCIE_DMA_READ_INT_CLEAR);
	return 0;
}
...
void dw_pcie_dma_set_list_ptr(struct dw_pcie *pci, u8 direction,
	u32 phy_list_addr)
{
	dw_pcie_writel_dbi(pci, PCIE_DMA_LLP_LOW, lower_32_bits(phy_list_addr));
	dw_pcie_writel_dbi(pci, PCIE_DMA_LLP_HIGH, upper_32_bits(phy_list_addr));
}
...
static int dw_pcie_dma_start_ll_write(struct dw_pcie *pci,
		struct dma_info *di, u32 phy_list_addr)
{
	if (pci && di) {
...
static int dw_pcie_dma_start_ll_read(struct dw_pcie *pci,
		struct dma_info *di, u32 phy_list_addr)
{
	if (pci && di) {
		if (di->rd_ch.status != DMA_CH_RUNNING) {
			...
			dw_pcie_writel_dbi(pci, PCIE_DMA_READ_INT_MASK, 0);
			dw_pcie_writel_dbi(pci, PCIE_DMA_READ_LINKED_LIST_ERR_EN, 0x10000);
			dw_pcie_writel_dbi(pci, PCIE_DMA_VIEWPORT_SEL, 0x80000000);
			dw_pcie_writel_dbi(pci, PCIE_DMA_CH_CONTROL1, 0x04000300);

			/* Set pointer to start of first list */
			dw_pcie_dma_set_list_ptr(pci, DMA_CH_READ, phy_list_addr);
			/* Ring doorbell */
...
int dw_pcie_dma_start_linked_list(struct dw_pcie *pci, struct dma_info *di,
	u32 phy_list_addr, u8 direction)
{
	if (direction == DMA_CH_WRITE)
		return dw_pcie_dma_start_ll_write(pci, di, phy_list_addr);

	/* else,  Read request */
	return dw_pcie_dma_start_ll_read(pci, di, phy_list_addr);
}
		...
		if (di->wr_ch.status == DMA_CH_RUNNING) {
			if (val_write & 0x10000) { /* Abort interrupt */
				...
				dw_pcie_dma_check_errors(pci,
					DMA_FLAG_WRITE_ELEM,
					&di->wr_ch.errors);
					dw_pcie_writel_dbi(pci, PCIE_DMA_WRITE_INT_CLEAR, 0x00FF0000);
#ifdef DMA_PTR_FUNC
				err_type = di->wr_ch.errors;
#endif
			} else { /* Done interrupt */
				dw_pcie_writel_dbi(pci, PCIE_DMA_WRITE_INT_CLEAR, 0x000000FF);
				/* Check channel list mode */
			}
			di->wr_ch.status = DMA_CH_STOPPED;
		} else
			dw_pcie_writel_dbi(pci, PCIE_DMA_WRITE_INT_CLEAR, 0x00FF00FF);
...
irqreturn_t dw_handle_dma_irq_read(struct dw_pcie *pci, struct dma_info *di, u32 val_read)
{
	...
	if (val_read) {
		if (di->rd_ch.status == DMA_CH_RUNNING) {
			...
			if (val_read & 0x80000) {
				/* Get error type */
				dw_pcie_dma_check_errors(pci, DMA_FLAG_READ_ELEM,
					&di->rd_ch.errors);
				dw_pcie_writel_dbi(pci, PCIE_DMA_READ_INT_CLEAR, 0x00FF0000);
#ifdef DMA_PTR_FUNC
				err_type = pp->rd_ch.errors;
#endif
			} else { /* Done interrupt */
				dw_pcie_writel_dbi(pci, PCIE_DMA_READ_INT_CLEAR, 0x000000FF);
				/* Check channel list mode */
			}
			di->rd_ch.status = DMA_CH_STOPPED;
		} else
			dw_pcie_writel_dbi(pci, PCIE_DMA_READ_INT_CLEAR, 0x00FF00FF);
