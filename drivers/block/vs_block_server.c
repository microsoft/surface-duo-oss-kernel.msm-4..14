...
vs_block_server_attach_block_device(struct block_server *server)
{
	...
	server->server.sector_size	= server->sector_size * VS_BLOCK_BLK_DEF_MIN_SECTORS;
	server->server.device_sectors	= bdev->bd_part->nr_sects / VS_BLOCK_BLK_DEF_MIN_SECTORS;
