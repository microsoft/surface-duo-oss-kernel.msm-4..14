...
#define DBGD(dev, fmt, arg...) \
		pr_debug("%s:%s:%d " fmt, (dev)->v4l2_dev.name, __func__, __LINE__, ## arg)
#define DBGI(dev, fmt, arg...) \
		pr_info("%s:%s:%d " fmt, (dev)->v4l2_dev.name, __func__, __LINE__, ## arg)
#define DBGE(dev, fmt, arg...) \
		pr_err("%s:%s:%d " fmt, (dev)->v4l2_dev.name, __func__, __LINE__, ## arg)
...
struct v4l2_loopback_device {
	...
	struct buffer_item buf_heads[MAX_BUFFERS];  // used to record enqueue buffer index
	struct list_head emptybufs_list; /* buffers in capture EQBUF */
	struct list_head fillbufs_list; /* buffers in output WRITE buffer */
	wait_queue_head_t fill_event; /* output fill buffer, then notify capture */
	spinlock_t fill_lock; /* used to protect emptybufs_list and fillbufs_list  */
...
static DEVICE_ATTR(status, S_IRUGO, attr_show_status, NULL);
		...
		struct v4l2_streamparm *parm)
{
	...
	MARK();
	dev = v4l2loopback_getdevice(file);
	DBGI(dev, "vidioc_s_parm called frate=%d/%d\n",
		...
		struct v4l2_requestbuffers *b)
{
	...
	case V4L2_MEMORY_MMAP:
		...
		DBGI(dev, "%s b->count:%d, used_buffers: %d", __func__, b->count, dev->used_buffers);
		return 0;
	default:
		return -EINVAL;
	}
		...
		void *private_data, struct v4l2_buffer *buf)
{
	...
	if (buf->index > max_buffers || buf->index < 0) {
	    DBGI(dev, "capture QBUF error: index: %d out of range: [0,%d) \n",
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		if (!dev->buf_heads[index].in_list) {
			list_add_tail(&(dev->buf_heads[index].list_head), &dev->emptybufs_list);
...
#define MAX_WAIT_TIME 2000  // ms
	...
	if (list_empty(&dev->fillbufs_list)) {
		wait_event_interruptible_timeout(dev->fill_event,
				(dev->current_status == STREAM_OFF) || !list_empty(&dev->fillbufs_list),
				msecs_to_jiffies(MAX_WAIT_TIME));
	}

	spin_lock(&dev->fill_lock);
	if (dev->current_status == STREAM_ON && !list_empty(&dev->fillbufs_list)) {
		bh = list_entry(dev->fillbufs_list.next, struct buffer_item, list_head);
		ret = bh->buffer_index;
		...
		void *private_data, enum v4l2_buf_type type)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		for (i = 0; i < dev->used_buffers; i++) {
			list_add_tail(&(dev->buf_heads[i].list_head), &dev->emptybufs_list);
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	wake_up_all(&dev->read_event);
	pr_info("leave v4l2_loopback_write(index: %d), used_buffers:%d \n", write_index, dev->used_buffers);
	return count;
}
#endif
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	spin_lock(&dev->fill_lock);
	if (dev->current_status == STREAM_OFF || list_empty(&dev->emptybufs_list)) {
		spin_unlock(&dev->fill_lock);
		DBGI(dev, "Warning: status:%d, empty buffer size:%d, skip write data\n",
					dev->current_status, list_empty(&dev->emptybufs_list));
		return count;
	}

	bh = list_entry(dev->emptybufs_list.next, struct buffer_item, list_head);
	list_del(&bh->list_head);
	...
	if (copy_from_user((void *)(dev->image + b->buffer.m.offset),
				(void __user *)buf, count)) {
		DBGI(dev, "failed copy_from_user() in write buf, could not write %zu\n",
			count);
		return -EFAULT;
	}
