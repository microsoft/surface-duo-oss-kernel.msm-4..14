...
static const struct file_operations gpu_proc_fops = {
        .open = gpu_proc_open,
        .read = seq_read,
        .llseek = seq_lseek,
...
{
	struct proc_dir_entry *gpu_arch_proc_dir;
	...
	if (kbdev->regulator) {
		...
		if (kbdev->gpu_vid == 0) {
			...
			{
				gpu_arch_proc_dir = proc_create("gpu_arch", S_IRUGO, NULL, &gpu_proc_fops);
				if (!gpu_arch_proc_dir)
				{
					dev_err(kbdev->dev, "Failed to create file gpu_arch!\n");
				}
			}
		}
