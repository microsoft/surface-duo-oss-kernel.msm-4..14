...
static const struct dma_fence_ops android_fence_ops;
...
struct sync_pt *sync_pt_create(struct sync_timeline *obj, int size)
{
	...
	dma_fence_init(&pt->base, &android_fence_ops, &obj->child_list_lock,
...
struct sync_fence *sync_fence_create_dma(const char *name, struct dma_fence *pt)
{
	struct sync_fence *fence;
	...
	if (dma_fence_add_callback(pt, &fence->cbs[0].cb, fence_check_cb_func))
...
static void sync_fence_add_pt(struct sync_fence *fence,
			      int *i, struct dma_fence *pt)
{
	fence->cbs[*i].sync_pt = pt;
	fence->cbs[*i].fence = fence;

	if (!dma_fence_add_callback(pt, &fence->cbs[*i].cb, fence_check_cb_func)) {
		dma_fence_get(pt);
		(*i)++;
	}
}
				    ...
				    struct sync_fence *a, struct sync_fence *b)
{
	...
	for (i = i_a = i_b = 0; i_a < a->num_fences && i_b < b->num_fences; ) {
		struct dma_fence *pt_a = a->cbs[i_a].sync_pt;
		struct dma_fence *pt_b = b->cbs[i_b].sync_pt;
...
static const char *android_fence_get_driver_name(struct dma_fence *fence)
{
...
static const char *android_fence_get_timeline_name(struct dma_fence *fence)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
...
static void android_fence_release(struct dma_fence *fence)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
	unsigned long flags;
...
static bool android_fence_signaled(struct dma_fence *fence)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
	int ret;
...
static bool android_fence_enable_signaling(struct dma_fence *fence)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
...
static int android_fence_fill_driver_data(struct dma_fence *fence,
					  void *data, int size)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
...
static void android_fence_value_str(struct dma_fence *fence,
				    char *str, int size)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
...
static void android_fence_timeline_value_str(struct dma_fence *fence,
					     char *str, int size)
{
	struct sync_pt *pt = container_of(fence, struct sync_pt, base);
	struct sync_timeline *parent = sync_pt_parent(pt);
...
static void sync_fence_free(struct kref *kref)
{
	...
	for (i = 0; i < fence->num_fences; ++i) {
		dma_fence_remove_callback(fence->cbs[i].sync_pt, &fence->cbs[i].cb);
		dma_fence_put(fence->cbs[i].sync_pt);
	}
...
static int sync_fill_pt_info(struct dma_fence *fence, void *data, int size)
{
