...
#define Q6ASM_DAI_COMPRESS_TX	1
#define Q6ASM_DAI_COMPRESS_RX	2
				    ...
				    struct q6asm_dai_data *pdata)
{
	...
	for_each_child_of_node(dev->of_node, node) {
		...
		ret = of_property_read_u32(node, "compress-direction", &dir);
		if (ret)
			continue;

		if (dir == Q6ASM_DAI_COMPRESS_RX)
			dai_drv->capture = empty_stream;
		else if (dir == Q6ASM_DAI_COMPRESS_TX)
