...
static bool msm_pcm_routing_route_is_set(u16 id, u16 fe_id)
{
	bool rc = false;
	...
	if (test_bit(fe_id, &msm_bedais[id].fe_sessions))
		rc = true;
				...
				struct snd_pcm_hw_params *params)
{
	...
	unsigned int id = rtd->dai_link->id;

	if (id >= MSM_BACKEND_DAI_MAX) {
		pr_err("%s: unexpected id %d\n", __func__, id);
		return -EINVAL;
	}

	mutex_lock(&routing_lock);
	msm_bedais[id].sample_rate = params_rate(params);
	msm_bedais[id].channel = params_channels(params);
	mutex_unlock(&routing_lock);
	return 0;
}
