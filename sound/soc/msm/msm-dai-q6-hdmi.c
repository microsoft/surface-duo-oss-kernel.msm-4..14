...
static	union afe_port_config hdmi_port_config;
		...
		struct snd_soc_dai *dai)
{
	...
	hdmi_port_config.hdmi_multi_ch.channel_allocation = 0,
	hdmi_port_config.hdmi_multi_ch.reserved  = 0,
//HDMI_RX = 8;
	printk("ADEBUG-DAI_HDMI::: %s HDMI_RX %d \n", __func__, dai->id);
	printk("ADEBUG-DAI_HDMI::: %s dai id %d hdmi rate %d config ch al %d reserved %d data type %d \n", __func__, HDMI_RX, dai_data->rate,
		dai_data->port_config.hdmi_multi_ch.channel_allocation,
		dai_data->port_config.hdmi_multi_ch.reserved,
		dai_data->port_config.hdmi_multi_ch.data_type);
	if (!test_bit(STATUS_PORT_STARTED, dai_data->status_mask)) {
...
//		break;
//	}
	printk("ADEBUG-DAI_HDMI: %s ret %d \n", __func__, rc);
	return rc;
}
