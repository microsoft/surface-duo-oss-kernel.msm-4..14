...
static inline void wcd_enable_clsh_block(struct snd_soc_codec *codec,
	...
	if ((enable && (++clsh_d->flyback_users == 1)) ||
	   (!enable && (--clsh_d->flyback_users == 0))) {
		...
		if (enable && (WCD9335_IS_1_1(clsh_d->codec_version))) {
			...
			snd_soc_write(codec, WCD9XXX_A_ANA_RX_SUPPLIES, vneg[0]);

			snd_soc_write(codec, WCD9XXX_A_ANA_RX_SUPPLIES, vneg[1]);
				   ...
				   u8 req_state, bool is_enable, int mode)
{
	...
	if (is_enable) {
		...
		if ((req_state == WCD_CLSH_STATE_HPHL) ||
		    (req_state == WCD_CLSH_STATE_HPHR)) {
			wcd_clsh_set_gain_path(codec, clsh_d, mode);
			wcd_clsh_set_flyback_mode(codec, mode);
			wcd_clsh_set_buck_mode(codec, mode);
		}
	} else {
...
{
	if (mode != CLS_H_NORMAL) {
		dev_err(codec->dev, "%s: mode: %d cannot be used for EAR\n",
			__func__, mode);
		return;
	}
		...
		u8 req_state, bool is_enable, int mode)
{
	...
	dev_err(codec->dev, "Invalid class H state machine requested\n"); 
	WARN_ON(1);
}
		...
		int int_mode)
{
	...
	case WCD_CLSH_EVENT_PRE_DAC:
		...
		if (!wcd_clsh_is_state_valid(new_state)) {
			dev_err(codec->dev, "Class-H not a valid new state:\n");
			return;
		}
		if (new_state == old_state) {
			dev_err(codec->dev, "Class-H already in requested state\n");
			return;
		}
	...
	case WCD_CLSH_EVENT_POST_PA:
		...
		if (new_state < NUM_CLSH_STATES_V2) {
			if (!wcd_clsh_is_state_valid(old_state)) {
				dev_err(codec->dev, "Invalid old state\n");
				return;
			}
			if (new_state == old_state) {
				dev_err(codec->dev,"Class-H already in requested state\n");
